...
# Digital Signatures via openSSE
# ---------------------------------------------------------
...
def generate_keys_if_missing():
    ...
    if not PRIV_KEY.exists():
        ...
        subprocess.run([OPENSSL, "genrsa", "-out", str(PRIV_KEY), "2048"], check=True)
    if not PUB_KEY.exists():
        #Extract publlic key
        subprocess.run([OPENSSL, "rsa", "-in", str(PRIV_KEY), "-pubout", "-out", str(PUB_KEY)], check=True)

def sign_file(input_path: str, sig_path: str):
        # Use SHA256 digest and RSA private key
        subprocess.run([OPENSSL, "dgst", "-sha256", "-sign", str(PRIV_KEY), "-out", sig_path, input_path], check=True)

def verify_file(input_path: str, sig_path: str) -> bool:
    #verify signature using public key 
    res = subprocess.run ([OPENSSL, "dgst", "-sha256", "-verify", str(PUB_KEY), "-signature", sig_path, input_path],
                          capture_output=True, text=True)
    return "Verified OK" in res.stdout

def action_signature_menu(role: str):
    print("== Digital Signatures (RSA cia OpenSSL) ==")
    ensure_openssl()
